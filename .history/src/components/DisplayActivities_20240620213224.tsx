	...
	return (
		<div>
			<table>
				...
				<tbody>
					<For each={active}>{(item,) => <GetDisplayItems item={item} activityType={props.activityType, props.loading} />}</For>
					<Show when={props.displayInactive && inactive.length > 0}>
						...
						<For each={inactive}>{(item,) => <GetDisplayItems item={item} activityType={props.activityType, props.loading} />}</For>
					</Show>
				</tbody>
			</table>
		</div>
